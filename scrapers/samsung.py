...
def _parse(data: dict) -> tuple[list[dict], int]:
    ...
    for j in data.get("jobPostings", []):
        ...
        if not title:
            continue
        jobs.append({
            "role_id":     f"samsung_{job_id}",
            "title":       title,
            "team":        "",
            "location":    j.get("locationsText", ""),
            "posted_date": _parse_posted(j.get("postedOn", "")),
            "url":         f"{BASE_URL}/Samsung_Careers{path}",
            "company":     "Samsung",
        })
    return jobs, total
...
def scrape() -> list[dict]:
    ...
    with httpx.Client(timeout=30, headers=HEADERS, follow_redirects=True) as client:
        client.get(f"{BASE_URL}/Samsung_Careers")
        ...
        while True:
            ...
            if total_pages is None and total > 0:
                ...
                print(f"[samsung] Total: {total}, pages: {total_pages}")

            new = [j for j in jobs if j["role_id"] not in seen]
            for j in new:
                seen.add(j["role_id"])
            all_jobs.extend(new)
            print(f"[samsung] Page {page}/{total_pages}: {len(new)} jobs (total {len(all_jobs)})")
    ...
    print(f"[samsung] Done. {len(all_jobs)} total jobs.")
    return all_jobs
