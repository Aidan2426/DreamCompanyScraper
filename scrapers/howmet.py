...
API_URL  = "https://fa-exty-saasfaprod1.fa.ocs.oraclecloud.com/hcmRestApi/resources/latest/recruitingCEJobRequisitions"
JOB_BASE = "https://fa-exty-saasfaprod1.fa.ocs.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1/job"
...
async def _fetch(session: AsyncSession, sem: asyncio.Semaphore, offset: int) -> list[dict]:
    async with sem:
        r = await session.get(
            API_URL,
            params={
                "finder": f"findReqs;siteNumber=CX_1,offset={offset}",
                "limit":  LIMIT,
...
async def scrape() -> list[dict]:
    async with AsyncSession(impersonate="chrome124") as session:
        r0 = await session.get(
            API_URL,
            params={"finder": "findReqs;siteNumber=CX_1,offset=0", "limit": LIMIT,
                    "expand": "requisitionList.secondaryLocations"},
            headers=HEADERS,
            timeout=30,
        )
    ...
    print(f"[howmet] Done. {len(jobs)} jobs.")
    return jobs
