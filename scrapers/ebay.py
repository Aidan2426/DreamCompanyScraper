...
SEARCH_URL = "https://jobs.ebayinc.com/us/en/search-results"
PAGE_SIZE = 10
HEADERS = {
        ...
        if html[i] == '[':
            ...
            if depth == 0:
                try:
                    ...
                    return [], total  # malformed page — caller will retry
                jobs = []
                for j in raw:
...
def scrape() -> list[dict]:
    ...
    with httpx.Client(timeout=30, headers=HEADERS, follow_redirects=True) as client:
        ...
        while True:
            ...
            if total is None and page_total > 0:
                ...
                print(f"[ebay] Total: {total}, pages: {total_pages}")

            if not jobs and total and from_offset + PAGE_SIZE < total:
                # Malformed HTML on this page — skip and continue
                print(f"[ebay] Page {page}/{total_pages}: parse error, skipping")
            else:
                new = [j for j in jobs if j["role_id"] not in seen]
                for j in new:
                    seen.add(j["role_id"])
                all_jobs.extend(new)
                print(f"[ebay] Page {page}/{total_pages}: {len(new)} jobs (total {len(all_jobs)})")

            if total and from_offset + PAGE_SIZE >= total:
                break
    ...
    print(f"[ebay] Done. {len(all_jobs)} total jobs.")
    return all_jobs
