...
JOB_BASE = "https://crowdstrike.wd5.myworkdayjobs.com/en-US/crowdstrikecareers"
HEADERS  = {
...
async def scrape() -> list[dict]:
    async with AsyncSession(impersonate="chrome124") as session:
        ...
        print(f"[crowdstrike] total={total}")

        sem  = asyncio.Semaphore(6)
        rest = await asyncio.gather(*[
            _fetch(session, off, sem)
    ...
    print(f"[crowdstrike] Done. {len(jobs)} jobs.")
    return jobs
