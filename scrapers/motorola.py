...
def _parse(data: dict) -> tuple[list[dict], int]:
    ...
    for j in data.get("jobPostings", []):
        ...
        if not title:
            continue
        jobs.append({
            "role_id":     f"motorola_{job_id}",
            "title":       title,
            "team":        "",
            "location":    j.get("locationsText", ""),
            "posted_date": _parse_posted(j.get("postedOn", "")),
            "url":         f"{BASE_URL}/Careers{path}",
...
def scrape() -> list[dict]:
    ...
    with httpx.Client(timeout=30, headers=HEADERS, follow_redirects=True) as client:
        ...
        while True:
            ...
            if total_pages is None and total > 0:
                ...
                print(f"[motorola] Total: {total}, pages: {total_pages}")

            new = [j for j in jobs if j["role_id"] not in seen]
            for j in new:
                seen.add(j["role_id"])
            all_jobs.extend(new)
            print(f"[motorola] Page {page}/{total_pages}: {len(new)} jobs (total {len(all_jobs)})")
    ...
    print(f"[motorola] Done. {len(all_jobs)} total jobs.")
    return all_jobs
