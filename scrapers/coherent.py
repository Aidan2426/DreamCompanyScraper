...
API_URL  = "https://hcwp.fa.us2.oraclecloud.com/hcmRestApi/resources/latest/recruitingCEJobRequisitions"
JOB_BASE = "https://hcwp.fa.us2.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1/job"
...
async def _fetch(session: AsyncSession, sem: asyncio.Semaphore, offset: int) -> list[dict]:
    async with sem:
        r = await session.get(
            API_URL,
            params={"finder": f"findReqs;siteNumber=CX_1,offset={offset}", "limit": LIMIT,
...
async def scrape() -> list[dict]:
    ...
    print(f"[coherent] Done. {len(jobs)} jobs.")
    return jobs
